...
public class LegalMove extends Data {
	...
	Cell[] allMove(int i, int j) {
		...
		if (boardData[i][j].equals("Hum")) {
			...
			if (i+1<=6 && j-1>=0 && (i+j==2|| i+j==4||i+j==6||i+j==8) && boardData[i+1][j-1].equals("##")) {
				moves[index++] = new Cell(i+1,j-1);
		...
		if (boardData[i][j].equals("BTom")) {
			...
			if ((j+1<=4 && (j==2 || i>=2) ) && boardData[i][j+1].equals("Tot")) {
				moves[index++] = new Cell(i,j+1);
			...
			if (i+1<=6 && j+1 <=4  && (i-j==4||i-j==2||i-j==0||i-j==-2) && boardData[i+1][j+1].equals("Tot")) {
				moves[index++] = new Cell(i+1,j+1);
			...
			if (i+1<=6 && j-1>=0 && (i+j==2|| i+j==4||i+j==6||i+j==8) && boardData[i+1][j-1].equals("Tot")) {
				moves[index++] = new Cell(i+1,j-1);
