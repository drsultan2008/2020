...
public class Data{
    ...
    Image playAsWhite;
    Image playAsBlack;
    Image pw,pb,rw,rb,nw,nb,bw,bb,qw,qb,kw,kb,Tot,Hum,BTom,background;
    ...
    Data(Stage stage)
    {
        ...
        bgImage = new Image(url+"board.jpg",maxWidth,maxHeight,true,true);
        playBgImage = new Image(url+"board.jpg",maxWidth,maxHeight,true,true);
        playAsBlack = new Image(url+"luotHum.png",200,100,true,true);
        playAsWhite= new Image(url+"luotTom.png",200,100,true,true);
//        pw = new Image(url+".png",squareSIze,squareSIze,true,true);
...
//        kb = new Image(url+"kb.png",squareSIze,squareSIze,true,true);
    ...
    void updateBoard()
    {
    	for (int i=0; i<7; i++) {
			for (int j=0; j<5; j++) {
				if (chessBoard.board[i][j].equals("Tot")) {
					boardView[j][i].setGraphic(new ImageView(Tot));
				}
				if (chessBoard.board[i][j].equals("Hum")) {
					boardView[j][i].setGraphic(new ImageView(Hum));
				}
				if (chessBoard.board[i][j].equals("BTom")) {
